...
>{
    ...
    get firstPos(): Position {
        return new Position(this.id, this.name, 0, 0);
    }
    ...
    protected isPositionInBounds(p: Position_like) {
        if (this.capacity <= 0) return false;
        if (p.zoneID != this.id) return false;
        return p.x >= 0 && p.x < this.boundX && (!this.boundY || (p.y < this.boundY && p.y >= 0))
    }
    validatePosition(p?: Position_like) {
        return !!p && p.zoneID === this.id && this.valid && this.boundX !== 0 && this.isPositionInBounds(p);
    }
    protected positionToIndex(p?: Position_like) {
    ...
    getAction_add(s : dry_system, c: Card,  p?: Position, cause : identificationInfo = actionFormRegistry.none()) : Action {
        ...
        return actionConstructorRegistry.a_move(s, c)(p)(cause)
    }
    ...
    getAction_remove(s : dry_system, c: Card, newPos: Position, cause : identificationInfo = actionFormRegistry.none()) {
        ...
        return actionConstructorRegistry.a_move(s, c)(newPos)(cause)
    }
    ...
    interact(s : dry_system, cause : identificationInfo, input : T_cull_interact extends [] ? undefined : {
        [K in keyof T_cull_interact] : T_cull_interact[K]["data"]
    }) : Action[] {
        return [];
    }
    ...
    getAllPos() : dry_position[] {
        return this.cardArr.map((_, index) => new Position(this.id, this.name, ...this.indexToPosition(index)))
    }
