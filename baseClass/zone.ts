...
import invalidOrderMap from "../errors/invalidOrderMap";
import unknownError from "../errors/unknownError";
import cardNotExist from "../errors/cardNotExist";
import zoneAttrConflict from "../errors/zoneAttrConflict";
import posChange from "../specificAction/posChange";
import invalidPosition from "../errors/invalidPosition";
import cardNotInApplicableZone from "../errors/cardNotInApplicableZone";
import zoneFull from "../errors/zoneFull";
...
class zone {
    ...
    //helper properties
    get isFull() {return (this.cardArr.length >= this.capacity)}
    get valid(){return this.id >= 0}

    //helper properties - quick attr access
    get posBound() : number[] {return this.attr.get("posBound") ?? []}
    set posBound(bound : number[]){this.attr.set("posBound", bound)};

    get priority() : number {return this.attr.get("priority") ?? -1}
    set priority(p : number) {this.attr.set("priority", p)} 

    get id() : number {return this.attr.get("index") ?? -1}
    get posLength() : number {return this.posBound.length}
    get shape() : number[] {return this.posBound}
    ...
    get firstPos() : position {return new position(this.id, ...utils.indexToPosition(0, this.shape))}
    //helper properties - boolean attributes
    get canReorderSelf() : boolean {return Boolean(this.attr.get("canReorderSelf"))} 
    get canMoveTo() : boolean {return Boolean(this.attr.get("canMoveTo"))} 
    get canMoveFrom() : boolean {return Boolean(this.attr.get("canMoveFrom"))} 
    get moveToNeedPosition() : boolean {return Boolean(this.attr.get("moveToNeedPosition"))} 
    get reorderNeedPosition() : boolean {return Boolean(this.attr.get("reorderNeedPosition"))}

    set canReorderSelf(value: boolean) {this.attr.set("canReorderSelf", value)} 
    set canMoveTo(value: boolean) {this.attr.set("canMoveTo", value)} 
    set canMoveFrom(value: boolean) {this.attr.set("canMoveFrom", value)} 
    set moveToNeedPosition(value: boolean) {this.attr.set("moveToNeedPosition", value)} 
    set reorderNeedPosition(value: boolean) {this.attr.set("reorderNeedPosition", value)}

    get capacity() : number {return (this.posLength) ? this.posBound.reduce((a, b) => a * b) : 0}
    ...
    findIndex(cid? : string){
        ...
        for(let index = 0; index < this.cardArr.length; index++){
            if(this.cardArr[index] && (this.cardArr[index] as card).id === cid) return index
        }
        return -1;
    }
    applyFuncToID<M>(func: (c : card) => M | undefined, cid? : string) : M | undefined{ //maybe obsolete
    ...
    getAction_add(isChain : boolean, c : card, p? : position){
        if(!this.canMoveTo){
            return new zoneAttrConflict(this.id, "moveTo", c.id).add("zone.ts", "getAction_add", 130)
        }
        if(!p && this.moveToNeedPosition){
            return new zoneAttrConflict(this.id, "moveTo", c.id).add("zone.ts", "getAction_add", 133)
        }
        if(!p) p = this.lastPos 
    ...
    getAction_remove(isChain : boolean, c : card, newPos? : position){ //probably not gonna be used
        if(!this.canMoveFrom){
            return new zoneAttrConflict(this.id, "moveFrom", c.id).add("zone.ts", "getAction_remove", 147)
        } 
    ...
    getAction_move(isChain : boolean, c : card, newPos : position){
        if(!this.canReorderSelf){
            return new zoneAttrConflict(this.id, "moveFrom", c.id).add("zone.ts", "getAction_move", 154)
        }
        let index = this.findIndex(c.id);
        if(index < 0 || !this.cardArr[index] || !this.validatePosition(c.pos)) 
            return new cardNotInApplicableZone(this.id, c.id).add("zone.ts", "getAction_move", 158)

        let toIndex = utils.positionToIndex(newPos.flat(), this.shape)
        if(toIndex < 0 || !this.validatePosition(newPos))
            return new invalidPosition(c.id, newPos).add("zone.ts", "getAction_move", 162)

        let swapTargetID : undefined | string = undefined 
        if(this.cardArr[toIndex]) swapTargetID = (this.cardArr[toIndex] as card).id
    ...
    getAction_shuffle(isChain : boolean){
        if(!this.canReorderSelf || !this.capacity){
            return new zoneAttrConflict(this.id, "shuffle").add("zone.ts", "getAction_shuffle", 181)
        }
    ...
    protected addToIndex(c : card, toIndex: number) : res{
        ...
        if (this.cardArr[toIndex]) return this.handleOccupied(c, toIndex, "addToIndex", 192)

        //let oldPos = c.pos; 
        c.pos = new position(this.id, ...utils.indexToPosition(toIndex, this.shape))
    ...
    add(c : card, p1 : position) : res{
        if(this.moveToNeedPosition){
            ...
            if(toIndex < 0 || !this.validatePosition(p1)) return this.handleInvalidPos((c) ? c.id : undefined, p1, "add", 204);
    ...
    move(c : card, p : position) : res {
        ...
        if(this.cardArr[toIndex]) return this.handleOccupied(c, toIndex, "move", 239);
    ...
    protected isValidOrderMap(orderMap: Map<number, number>, arrayLength: number): boolean {
        ...
        for (const [oldIndex, newIndex] of orderMap.entries()) {
            ...
            if (oldIndex < 0 || oldIndex >= arrayLength || newIndex < 0 || newIndex >= arrayLength) {
                return false;
            }
    ...
    shuffle(orderMap : Map<number, number>) : res{
        if(!this.canReorderSelf || !this.capacity) return this.handleNoReorder("shuffle", 298)
    ...
    getZoneRespond(a : action, isChain : boolean) : action[]{return []}
    ...
    activateEffect(cidx : number, eID : number, system : dry_system, a : action) : res{
    ...
    handleNoMoveTo(c : card, func: string, line? : number) : res {
        return [new zoneAttrConflict(this.id, "move card to this zone").add(this.name, func, line), undefined]
    }
    handleNoMoveFrom(c : card, func: string, line? : number) : res {
        return [new zoneAttrConflict(this.id, "move card away from this zone").add(this.name, func, line), undefined]
    }
    handleNoReorder(func: string, line? : number) : res{
        return [new zoneAttrConflict(this.id, "move card within this zone").add(this.name, func, line), undefined];
    }
    ...
    handleCardNotInApplicableZone(c : card, func: string, line? : number) : res{
        return [new cardNotInApplicableZone(this.id, c.id).add(this.name, func, line), undefined]
    }
    handleInvalidOrderMap(orderMap : Map<number, number>, func : string, line? : number) : res{
        return [new invalidOrderMap(orderMap).add(this.name, func, line), undefined]
    }
    protected handleOccupiedSwap(c : card, index : number, func: string, line? : number) : res {
        ...
        if(c.pos.zoneID != this.id) return this.handleCardNotInApplicableZone(c, func, line)
    ...
    forceCardArrContent(newCardArr : card[]){
        ...
        this.cardArr.forEach((i, index) => {
            let p : position = new position(this.id, ...utils.indexToPosition(index, this.shape));
            (i as card).pos = p
            console.log("forcing card in index " +  index + " to pos " + p.toString() + " whose validity is " + p.valid)
        })
    }
}
