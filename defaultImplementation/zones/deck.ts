...
class Deck extends zone_stack<[], [inputData_zone]> {
    ...
    override getInput_interact(s: dry_system, cause: identificationInfo) {
        return Request.specificType(s, this, zoneRegistry.z_hand).ofSamePlayer(this).once()
    }

    override interact(s: dry_system, cause: identificationInfo, input: [dry_zone]): Action[] {
        const hand = input[0];
    ...
    draw(s : dry_system, a : Action<"a_draw">, hand : Hand) : res {
        ...
        if(attr.actuallyDraw){
            ...
            if(card){
                res[1] = [actionConstructorRegistry.a_move(s, card)(hand.top)(actionFormRegistry.zone(s, d))]
