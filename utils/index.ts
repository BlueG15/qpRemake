...
class utils {
    ...
    /** 
     * Apply many partial object (patches) onto an original object, this returns a hard copy instead of modifying the original.
     * The original may have more keys than the patch, in this case only the keys in the original is kept.
     * Patches are apply sequentially, the last would win in the case of a conflict.
     * For type inference, T is infered from the patches actually, not original for some reason
     * */
    static patch<T extends Record<string, any>>(
        original: T,
        ...
        for (const key of keys) {
            // Prtotype polluton prevention thingy
            if (key === '__proto__' || key === 'constructor' || key === 'prototype') continue;

            for (const patch of patches) {
                if (patch && Object.hasOwn(patch, key)) {
                    res[key] = patch[key as keyof T]
                    break;
                }
            }

            if (!Object.hasOwn(res, key))
                res[key] = original[key as keyof T];
        }
