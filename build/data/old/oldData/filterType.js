...
exports.CustomGroup = void 0;
exports.getGroupNameFromID = getGroupNameFromID;
const serverOrigin = window.origin;
const getEffIco = (i) => serverOrigin + "/icons/" + i + ".png";
const getLvlIco = (i) => `https://placehold.co/128/0000/FFF?text=${i}&font=Montserrat`;
var FILTERTYPEOPTION;
(function (FILTERTYPEOPTION) {
    FILTERTYPEOPTION["EFFECT"] = "effect";
    FILTERTYPEOPTION["RARITY"] = "rarity";
    FILTERTYPEOPTION["LEVEL"] = "level";
    FILTERTYPEOPTION["DECK"] = "deck";
...
class CustomGroup {
    ...
    addToList(filter, group) {
        if (this.filters[filter]) {
            if (!this.isActive(group))
                this.filters[filter].push(group);
        }
        return this.getGroupString();
    }
    removeFromList(filter, group) {
        if (this.filters[filter]) {
            if (this.isActive(group))
    ...
    getActiveGroups(type) {
        return this.filters[type];
    }
    acceptAll(type) {
        if (type && this.filters[type]) {
            this.filters[type] = Object.values(filterType[type].groups).map((a) => a.id);
        }
        else {
...
exports.CustomGroup = CustomGroup;
const effectFilter = (card, id) => {
    if (id === "none")
        return card.effectCount === 0;
    const eff = card.effects.map((e) => e.effectType);
    const empty = [];
    const allFFS = empty.concat(...eff);
    return allFFS.includes(id);
};
const rarityFilter = (card, id) => {
    return card.rarity === id;
};
const levelFilter = (card, id) => {
    var _a;
    return id === "level-" + ((_a = card.level) !== null && _a !== void 0 ? _a : 0);
};
const archtypeFilter = (card, id) => {
    return card.belongTo === id;
};
const upgradeabilityFilter = (card, id) => {
    if (id === "upgradable") {
...
function getGroupNameFromID(filterID, id) {
