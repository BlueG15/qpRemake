...
function evaluateNumericOperation(opCode, a, b) {
    switch (opCode) {
        case "+":
            return a + b;
        case "-":
...
function evaluate_node(node, inputArr) {
    var _a, _b, _c, _d;
    if (node.length == 3) {
        if (node[1].length == 1 && node[2].length == 1) {
            let num1 = isNaN(Number(node[1][0])) ? ((_a = inputArr[node[1][0].charCodeAt(0) - 'a'.charCodeAt(0)]) !== null && _a !== void 0 ? _a : NaN) : Number(node[1][0]);
            let num2 = isNaN(Number(node[2][0])) ? ((_b = inputArr[node[2][0].charCodeAt(0) - 'a'.charCodeAt(0)]) !== null && _b !== void 0 ? _b : NaN) : Number(node[2][0]);
            return evaluateNumericOperation(node[0], num1, num2);
        }
        if (node[1].length == 1) {
            let num1 = isNaN(Number(node[1][0])) ? ((_c = inputArr[node[1][0].charCodeAt(0) - 'a'.charCodeAt(0)]) !== null && _c !== void 0 ? _c : NaN) : Number(node[1][0]);
            let num2 = evaluate_node(node[2], inputArr);
            return evaluateNumericOperation(node[0], num1, num2);
        }
        if (node[2].length == 1) {
            let num1 = evaluate_node(node[1], inputArr);
            let num2 = isNaN(Number(node[2][0])) ? ((_d = inputArr[node[2][0].charCodeAt(0) - 'a'.charCodeAt(0)]) !== null && _d !== void 0 ? _d : NaN) : Number(node[2][0]);
            return evaluateNumericOperation(node[0], num1, num2);
        }
        let num1 = evaluate_node(node[1], inputArr);
        let num2 = evaluate_node(node[2], inputArr);
...
class numericModule extends parser_1.parserModule {
    constructor() {
        super(...arguments);
        this.cmdName = ['numeric'];
    ...
    evaluate(cmd, args, option, raw) {
        ...
        try {
            ...
            return [new parser_1.numberComponent(NaN, a.message, cmd, raw)];
        }
        let res;
        try {
        ...
        return [new parser_1.numberComponent(res, undefined, cmd, raw)];
    }
}
exports.default = numericModule;
